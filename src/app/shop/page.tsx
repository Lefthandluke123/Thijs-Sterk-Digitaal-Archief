...
export default function ShopPage() {
  ...
  const artworksQuery = useMemoFirebase(() => {
    ...
    // Voor het prototype tonen we alle werken die in de shop staan OF featured zijn
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        <header className="mb-20 text-center space-y-4 max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20 mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">Museumwinkel</span>
          </div>
        ...
        {artworks && artworks.length > 0 ? (
              ...
              <Link key={art.id} href={`/shop/${art.id}`} className="group">
                <Card className="border-none shadow-none bg-transparent space-y-6">
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    <p className="text-[12px] font-bold text-accent pt-2">v.a. €{art.pricePostcard || '2.50'}</p>
                  </div>
                </Card>
              </Link>
            ))}
          ...
          <div className="py-32 text-center space-y-6 bg-secondary/5 rounded-[3rem] border border-dashed border-border">
            ...
            <div className="space-y-2">
              <h3 className="font-headline text-2xl font-light">De winkel wordt momenteel ingericht</h3>
              <p className="text-sm text-muted-foreground uppercase tracking-widest font-bold">Kom binnenkort terug voor de collectie reproducties</p>
            </div>
          </div>
        )}
      </div>
