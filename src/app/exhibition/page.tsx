  ...
  return (
    <main className="h-screen w-full bg-white overflow-hidden flex flex-col relative pt-16 md:pt-32">
      <div className="absolute top-10 left-0 right-0 z-40 flex justify-center px-6">
         <div className="bg-white/80 backdrop-blur-md border rounded-full px-6 py-2.5 flex items-center gap-6 shadow-xl overflow-x-auto no-scrollbar max-w-[90vw]">
            {rooms.map((r: any) => (
              <button 
                key={r.id} 
                onClick={() => {
                ...
                className={cn(
                  "text-[12px] font-bold uppercase tracking-widest whitespace-nowrap transition-all", 
      ...
      <div className="absolute top-44 left-1/2 -translate-x-1/2 z-20 text-center pointer-events-none opacity-90 w-full px-4">
        <h1 className="text-black/60 font-headline text-4xl md:text-5xl font-medium italic tracking-tight">{activeRoom?.title}</h1>
      </div>
      ...
      <div className="relative flex-1 flex items-center justify-center overflow-hidden">
        ...
        >
          <div className="flex gap-24 px-[15vw] items-center pt-8">
            {artworks?.length === 0 ? (
              ...
              artworks?.map((art: any) => (
                <div key={art.id} className="relative group shrink-0 flex items-center justify-center" onClick={() => setSelectedArtwork(art)}>
                  <div className="relative flex flex-col bg-white shadow-2xl border border-black/[0.03] cursor-pointer transition-all duration-700 hover:scale-[1.01] items-center justify-center overflow-hidden max-w-[450px]">
                     <div className="p-6 pb-4 flex items-center justify-center bg-gray-50/30 w-full">
                        {art.image && (
                          <img 
                            src={art.image} 
                            className="relative block max-h-[45vh] w-auto object-contain mx-auto" 
                            style={{ filter: `brightness(${art.brightness || 1})` }}
                            alt={art.displayTitle || art.title} 
                          />
                        )}
                     </div>
                     <div className="w-full px-6 py-6 border-t border-black/[0.03] bg-white text-center">
                        <h3 className="text-black text-[10px] font-bold uppercase tracking-[0.2em] mb-1 truncate">{art.displayTitle || art.title}</h3>
            ...
            {nextRoom && (
              <div className="shrink-0 w-[40vw] flex flex-col items-center justify-center text-center opacity-40 group hover:opacity-100 transition-opacity">
                 <h4 className="text-[10px] font-bold uppercase tracking-[0.5em]">Volgende Zaal</h4>
                 <button 
                  onClick={() => {
                    setScrollX(0);
                    router.push(`/exhibition?room=${nextRoom.slug || nextRoom.id}`);
                  }} 
                  className="mt-10 px-8 py-3.5 rounded-full bg-accent text-accent-foreground text-[9px] font-bold uppercase tracking-[0.25em] shadow-xl hover:scale-105 transition-all flex items-center gap-3"
                 >
      ...
      <div className="absolute bottom-10 left-0 right-0 z-30 flex justify-center gap-12 items-center">
        <button onClick={() => handleStep(-800)} className="p-4 rounded-full bg-white/60 backdrop-blur-md border border-black/5 text-black/40 hover:text-accent transition-all active:scale-90 shadow-lg group"><ChevronLeft className="w-8 h-8 transition-transform group-hover:-translate-x-1" /></button>
        <div className="flex flex-col items-center gap-2">
           <span className="text-[7px] font-black uppercase tracking-[0.5em] opacity-30">Wandel door de collectie</span>
           <div className="w-32 h-0.5 bg-black/5 rounded-full" />
        </div>
        <button onClick={() => handleStep(800)} className="p-4 rounded-full bg-white/60 backdrop-blur-md border border-black/5 text-black/40 hover:text-accent transition-all active:scale-90 shadow-lg group"><ChevronRight className="w-8 h-8 transition-transform group-hover:translate-x-1" /></button>
      </div>
...
export default function ExhibitionPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center bg-white"><Loader2 className="animate-spin text-accent" /></div>}>
      <ExhibitionContent />
    </Suspense>
  );
}
