...
export function GalleryClient({ initialSeries }: { initialSeries: string | null }) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const { t, language } = useLanguage();
  const seriesParam = searchParams.get('series') || initialSeries;
  
  const [selectedArtwork, setSelectedArtwork] = useState<any | null>(null);
  const [activeSeries, setActiveSeries] = useState<string | null>(seriesParam);
