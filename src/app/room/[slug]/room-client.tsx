...
import { ArrowLeft, Info, ChevronLeft, ChevronRight, Eye, EyeOff } from 'lucide-react';
...
export function RoomClient({ artworks, roomTitle }: RoomClientProps) {
  ...
  return (
    ...
    >
      <div className={cn(
        "absolute top-0 left-0 right-0 z-[120] p-6 md:p-10 flex items-center justify-between pointer-events-none transition-all duration-700",
        isZenMode ? "opacity-0 translate-y-[-20px]" : "opacity-100"
      )}>
      ...
      {/* Zen Toggle - Altijd subtiel bereikbaar */}
      <button 
        onClick={toggleZen}
        className={cn(
          "absolute top-8 left-8 z-[130] p-4 rounded-full backdrop-blur-md border border-black/5 transition-all shadow-lg",
          isZenMode ? "bg-accent text-accent-foreground opacity-40 hover:opacity-100" : "bg-white/80 text-foreground",
          !isZenMode && "md:hidden" // Op desktop is de back-knop er al, op mobiel is dit de enige linkerboven knop
        )}
      >
        {isZenMode ? <Eye className="w-5 h-5" /> : <EyeOff className="w-5 h-5" />}
      </button>

      {/* Extra Zen Toggle voor Desktop als de rest van de UI er wel is */}
      {!isZenMode && (
        <button 
          onClick={toggleZen}
          className="hidden md:flex absolute top-32 left-10 z-[120] p-3 rounded-full bg-white/40 hover:bg-white/80 transition-all border border-black/5"
          title="Verberg interface"
        >
      ...
      <div className={cn(
        "absolute inset-x-0 top-1/2 -translate-y-1/2 z-[110] flex justify-between px-8 pointer-events-none transition-all duration-700",
      ...
      <div className={cn(
        ...
        showMetadata && !isZenMode ? "h-auto min-h-[30vh] opacity-100 py-12 translate-y-0" : "h-0 opacity-0 pointer-events-none translate-y-12"
      )}>
      ...
      <div className={cn(
        "absolute bottom-8 left-1/2 -translate-x-1/2 z-[115] flex gap-2 transition-all duration-700",
        isZenMode ? "opacity-0" : "opacity-100"
      )}>
        {artworks.map((_, i) => (
