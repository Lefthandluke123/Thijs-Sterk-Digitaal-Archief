      ...
      <DialogContent className="max-w-[100vw] w-full h-[100vh] p-0 flex flex-col bg-black border-none rounded-none overflow-hidden outline-none shadow-none fixed inset-0 z-[100]">
        ...
        <div className="relative flex-1 bg-black overflow-hidden">
          ...
          <div className={cn("absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-8 pointer-events-none z-20 transition-opacity", isAnimating ? "opacity-0" : "opacity-100")}>
            {onPrev && (
              <button onClick={(e) => { e.stopPropagation(); onPrev(); }} className="p-5 rounded-full bg-black/20 backdrop-blur-md pointer-events-auto hover:bg-black/40 transition-all border border-white/10 group/btn"><ChevronLeft className="w-10 h-10 text-white opacity-40 group-hover/btn:opacity-100 transition-opacity" /></button>
            )}
            {onNext && (
              <button onClick={(e) => { e.stopPropagation(); onNext(); }} className="p-5 rounded-full bg-black/20 backdrop-blur-md pointer-events-auto hover:bg-black/40 transition-all border border-white/10 group/btn"><ChevronRight className="w-10 h-10 text-white opacity-40 group-hover/btn:opacity-100 transition-opacity" /></button>
            )}
          </div>
          ...
          <div className={cn("absolute top-8 right-8 z-[110] flex items-center gap-4 transition-opacity", isAnimating ? "opacity-0 pointer-events-none" : "opacity-100")}>
             ...
             {audio && (
               <button onClick={toggleAudio} className={cn("p-4 rounded-full backdrop-blur-xl border border-white/10 transition-all flex items-center gap-3", isPlaying ? "bg-accent text-accent-foreground" : "bg-black/40 text-white")}>
             ...
             <button onClick={() => setShowMetadata(!showMetadata)} className={cn("p-4 rounded-full backdrop-blur-xl border border-white/10 transition-all", showMetadata ? "bg-accent text-accent-foreground" : "bg-black/40 text-white hover:bg-black/60")}><Info className="w-5 h-5" /></button>
             
             <DialogClose className="p-4 bg-black/40 backdrop-blur-xl rounded-full text-white hover:bg-destructive transition-all border border-white/10"><X className="w-5 h-5 opacity-60" /></DialogClose>
          </div>
