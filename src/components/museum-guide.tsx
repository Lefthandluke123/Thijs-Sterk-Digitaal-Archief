        ...
        <div className="bg-primary p-12 text-primary-foreground relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-6">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/20 border border-accent/30">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">Safe Harbor Architectuur</span>
            </div>
            <h1 className="font-headline text-4xl md:text-6xl font-light leading-tight">
              Welkom in het <span className="italic">Digitaal Retrospectief</span>
            </h1>
            <p className="text-xl text-primary-foreground/70 font-light max-w-2xl leading-relaxed">
              Dit museum is een levend archief. Een plek waar de essentie van Licht, Ruimte en Water wordt bewaard voor de toekomst.
            </p>
          </div>
        </div>

        <div className="p-8 md:p-12 space-y-16">
          <Section title="Het Concept" icon={Sparkles}>
            <p className="text-muted-foreground leading-relaxed italic border-l-4 border-accent pl-6">
              "Het oeuvre van Thijs Sterk (1913-1982) omvat meer dan 325 werken. Dit platform ontsluit de collectie in museale kwaliteit, waarbij elk detail tot op de verfkorrel bestudeerd kan worden via onze Deep Zoom technologie."
            </p>
          </Section>

          <div className="grid md:grid-cols-2 gap-12">
            <Section title="Navigatie Gids" icon={Layout}>
              <Item 
                icon={Zap} 
                label="De Tour" 
                desc="Een meeslepende, horizontale wandeling door de zalen. Gebruik uw muis, de knoppen onderin of de pijltjestoetsen om te wandelen." 
              />
              <Item 
                icon={Globe2} 
                label="Zalen" 
                desc="Directe toegang tot thematische collecties zoals 'Polder', 'Bretagne' of 'Monumentaal'." 
              />
              <Item 
                icon={MousePointer2} 
                label="Uw Zaal (Curator)" 
                desc="Stel uw eigen expositie samen. Combineer tags zoals 'Olieverf' en 'Haven' om uw persoonlijke selectie te tonen." 
              />
              <Item 
                icon={ShoppingBag} 
                label="Winkel" 
                desc="Haal een 'Sterk' in huis. Van hoogwaardige posters tot gecertificeerde Fine Art prints." 
              />
            </Section>

            <Section title="De Viewer (Deep Zoom)" icon={ZoomIn}>
              <Item 
                icon={ZoomIn} 
                label="De Grote Loep (+/-)" 
                desc="Onze krachtigste tool. Zoomt met één klik direct 5x in op de details van het schilderij." 
              />
              <Item 
                icon={Info} 
                label="Informatie (i)" 
                desc="Toont de verborgen informatiebalk met het jaartal, de techniek en de afmetingen van het werk." 
              />
              <Item 
                icon={ArrowRight} 
                label="Snel Bladeren" 
                desc="Gebruik de pijlen links en rechts om naar het volgende werk in de zaal te gaan zonder de viewer te sluiten." 
              />
            </Section>
          </div>

          <Section title="Bediening & Sneltoetsen" icon={Keyboard}>
          ...
          <div className="text-center pb-8">
             <p className="text-[10px] font-black uppercase tracking-[0.4em] text-accent/50 italic">
               Een leven gewijd aan de essentie van het licht.
             </p>
          </div>
        </div>
      </DialogContent>
