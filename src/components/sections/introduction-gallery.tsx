 ...
 * Geoptimaliseerd voor stabiele compilatie en parallel laden.
 */
export function IntroductionGallery() {
  ...
  const curatedArtworks = useMemo(() => {
    ...
    // Snelle, niet-blokkerende selectie van diverse werken
    const unique = allArtworks.slice(0, 4);
    return unique;
  }, [allArtworks]);

  const getArtworkRoomSlug = useCallback((item: any) => {
  ...
  const navigateArtwork = useCallback((direction: 'next' | 'prev') => {
    if (!selectedArtwork || !curatedArtworks.length) return;
      ...
      <div className="container max-w-7xl mx-auto">
        <div className="text-center mb-24 space-y-6">
          <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-accent/5 border border-accent/10 mx-auto">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-accent">{t('homeIntroBadge')}</span>
          </div>
                  ...
                  <div key={item.id} className="group flex flex-col space-y-6">
                    ...
                    <div className="space-y-4 text-center px-2">
                      <div className="space-y-1.5">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground opacity-60">{item.year}</p>
                      </div>
                      <Link 
                        href={`/room/${getArtworkRoomSlug(item)}`}
